...
SELECT ГРУППА, 
    CASE 
        WHEN Н_ЛЮДИ.ДАТА_СМЕРТИ = '9999-09-09' OR NULL THEN AVG(AGE(Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ))
        ELSE AVG(AGE(Н_ЛЮДИ.ДАТА_СМЕРТИ, Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ))
    END AS "Средний возраст"
FROM Н_УЧЕНИКИ
JOIN Н_ОБУЧЕНИЯ ON Н_ОБУЧЕНИЯ.ЧЛВК_ИД = Н_УЧЕНИКИ.ЧЛВК_ИД
JOIN Н_ЛЮДИ ON Н_ЛЮДИ.ИД = Н_ОБУЧЕНИЯ.ЧЛВК_ИД
WHERE Н_ЛЮДИ.ДАТА_СМЕРТИ IS NOT NULL
GROUP BY ГРУППА, Н_ЛЮДИ.ДАТА_СМЕРТИ;
